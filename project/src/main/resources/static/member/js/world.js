...
const rendernews = async(data) => {
    const respones=await fetch("http://localhost:8082/user/follow/world_news/"+user.userId);
...
const fetchAndRenderData = async () => {
	const fullUrl = 'http://localhost:8082/news/world';
	const response = await fetch(fullUrl); // 等待 fetch 請求完成
	const { state, message, data } = await response.json(); // 等待回應本文內容
	rendernews(data);
...
const handletonewspage = async (id) => {
	const Url = 'http://localhost:8082/news/world/' + id+"/"+user.gender;
